...
function MetricCard({ label, value, hint, accent }: {
  ...
  return (
    <div className={["rounded-[14px] p-4 lg:rounded-[16px] lg:p-5", accent ? "bg-[#ff6a2b] text-white" : "bg-[#f6f7f9]"].join(" ")}>
      <p className={`text-[14px] font-medium lg:text-[15px] ${accent ? "text-white/80" : "text-[#667085]"}`}>{label}</p>
      <p className={`mt-2 text-[20px] font-bold tracking-[-0.03em] lg:text-[22px] ${accent ? "text-white" : "text-[#111827]"}`}>{value}</p>
      {hint && <p className={`mt-1 text-[11px] lg:text-[12px] ${accent ? "text-white/70" : "text-[#98a2b3]"}`}>{hint}</p>}
    </div>
  );
}
...
export default async function VendasPage() {
  const [
    ...
    supabaseAdmin.from("sale_items").select("product_id, quantidade, sale_id, lucro_liquido_rs"),
    supabaseAdmin
      .from("sales")
  ...
  for (const s of (saleItemsAll ?? []) as any[]) sold[s.product_id] = (sold[s.product_id] ?? 0) + s.quantidade;
  ...
  const trintaStr = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const salesMes = sales.filter((s) => s.data_venda >= trintaStr);
  const faturamentoMes = salesMes.reduce((s, v) => s + v.total_rs, 0);
    ...
    <AppShell title="Vendas" subtitle="Registro de vendas e baixa de estoque">
      <div className="space-y-4 lg:space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <MetricCard label="Lucro 30d" value={brl(lucroMes)} hint={faturamentoMes > 0 ? `${pct(lucroMes / faturamentoMes)} margem` : undefined} />
        ...
        <div className="grid items-start gap-4 xl:grid-cols-[1.1fr_0.9fr]">
          ...
          <Section
            title="Histórico"
            right={<span className="rounded-full bg-white px-3 py-1 text-[12px] font-medium text-[#667085]">{sales.length} vendas</span>}
          >
