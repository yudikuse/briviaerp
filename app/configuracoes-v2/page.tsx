...
function MetricCard({ label, value, accent, hint }: { label: string; value: string; accent?: boolean; hint?: string }) {
  return (
    <div className={["rounded-[14px] p-4 lg:rounded-[16px] lg:p-5", accent ? "bg-[#ff6a2b] text-white" : "bg-[#f6f7f9] text-[#111827]"].join(" ")}>
...
function MobileList({ rows }: { rows: { label: string; value: string }[] }) {
  return (
    <div className="overflow-hidden rounded-[12px] bg-white lg:hidden">
      {rows.map((row, idx) => (
        <div key={row.label} className={["flex items-start justify-between gap-4 px-4 py-3", idx !== rows.length - 1 ? "border-b border-[#edf0f4]" : ""].join(" ")}>
          <div className="text-[14px] text-[#111827]">{row.label}</div>
          <div className="shrink-0 text-right text-[14px] font-medium text-[#111827]">{row.value}</div>
        </div>
      ))}
    </div>
  );
          ...
          {rows.map((row, idx) => (
            <tr key={idx} className="border-t border-[#f1f4f8]">
              {row.map((cell, cellIdx) => (
                <td key={cellIdx} className="px-4 py-4 text-[14px] text-[#111827]">{cell}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
export default async function ConfiguracoesV2Page() {
  const [{ data: settings }, { data: fixedCosts }] = await Promise.all([
    ...
    supabaseAdmin.from("fixed_costs").select("id, descricao, valor_mensal, ativo, ordem").order("ordem", { ascending: true }),
  ]);

  const totalFixedCosts = (fixedCosts ?? []).filter((i) => i.ativo).reduce((s, i) => s + Number(i.valor_mensal ?? 0), 0);
  ...
  const marginBase = Number(settings?.minimum_target_margin_pct ?? 0) > 0
    ...
    <AppShell title="Configurações" subtitle="">
      <div className="space-y-4 lg:space-y-5">
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <MetricCard label="Faturamento necessário" value={brl(requiredRevenue)} hint={marginBase > 0 ? `Base ${pct(marginBase)}` : "Defina a margem alvo"} />
        </div>
        ...
        <div className="grid items-start gap-4 xl:grid-cols-[1.08fr_0.92fr]">
          ...
          <Section title="Objetivos">
            <GoalsForm monthlyProfitGoal={monthlyProfitGoal} cashGoal={cashGoal} purchaseGoal={purchaseGoal} />
          </Section>
        </div>

        <Section title="Custos fixos" right={
          <div className="rounded-full bg-white px-3 py-1 text-[12px] font-medium text-[#667085]">
            {fixedCosts?.length ?? 0} itens
          </div>
        }>
          <MobileList rows={(fixedCosts ?? []).map((i) => ({ label: i.descricao, value: `${brl(Number(i.valor_mensal ?? 0))} • ${i.ativo ? "Ativo" : "Inativo"}` }))} />
          <DesktopTable
            headers={["Item", "Valor mensal", "Status"]}
            rows={(fixedCosts ?? []).map((i) => [i.descricao, brl(Number(i.valor_mensal ?? 0)), i.ativo ? "Ativo" : "Inativo"])}
          />
        </Section>
      </div>
    </AppShell>
